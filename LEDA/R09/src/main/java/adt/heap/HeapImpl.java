...
public class HeapImpl<T extends Comparable<T>> implements Heap<T> {
	...
	public T[] heapsort(T[] array) {
		buildHeap(array);
		T[] arrayFinal = Arrays.copyOf(array, array.length);
		// Se for min adiciona normal no array.
		if (getType().equals("min")) {
			for (int i = 0; i < array.length; i++) {
				arrayFinal[i] = this.extractRootElement();
			}
		}
		// Se for max adiciona de trás para frente no array.
		else {
