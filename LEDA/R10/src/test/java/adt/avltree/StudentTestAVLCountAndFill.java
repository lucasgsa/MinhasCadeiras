...
public class StudentTestAVLCountAndFill {
	...
	public void testFillWithoutRebalance() {
		...
		assertEquals(0, tree1.LRcount());
		assertEquals(0, tree1.RLcount());
		AVLTreeVerifierImpl<Integer> avlv = new AVLTreeVerifierImpl<>(tree1);
		assertTrue(avlv.isAVLTree());
		assertTrue(avlv.isBST());
		
		AVLCountAndFillImpl<Integer> treeT1 = new AVLCountAndFillImpl<Integer>();
		Integer[] keys1 = {8, 4, 12, 2, 6, 10, 14, 1};
		Integer[] keys2 = {3, 5, 7, 9, 11, 13, 15};
		treeT1.fillWithoutRebalance(keys1);
		treeT1.fillWithoutRebalance(keys2);
		assertEquals(treeT1.preOrder(), tree1.preOrder());
